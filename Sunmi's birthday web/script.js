...
if ( entry.target.classList.contains('message-card')) {
    ...
    elementsToObserrve.forEach(element => {
...
function animateMessageText(){
    ...
    messageTexts.forEach((texts, index) => {
        setTimeout(() => {
            texts.classList.add('fade-in-animate');
        }, index * 500);
    });
}
...
// toogle like functionality for photos
function toggleLike(button){
    const heartIcon = button.querySelectorAll('.heart-icon');
    button.classList.toggle('.liked');

    if(buttton.classList.contains('liked')){
...
window.addEventListener('scroll', ()=> {
    ...
    if(hero){
        hero.style.transfers = `translateY(${scrolled * parallaxSpeed}px)`;
    }
    ...
    particles.forEach((particle, index)=> {
        ...
        particle.style.transfers = `translateY(${scrolled * speed}px)`;
    });
});

// Add movement effect to hero section
...
const photoObserver = new IntersectionObserver((entries)=> {
    entries.forEach(entry=> {
        if (entry.isIntersecting){
            const img = CustomElementRegistry.target.querySelector('img');
            if(img){
